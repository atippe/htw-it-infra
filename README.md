...
```mermaid
flowchart TB
    %% Define different arrow styles
    linkStyle default stroke-width:2px
    
    subgraph GithubActions["GitHub Actions"]
        GH[GitHub Repository] ==> |Push to main| W1[Build Workflow]
        W1 ==> |Triggers| W2[Deploy Workflow]
        W2 ==> |Deploys| CF[CloudFormation]
    end
    
    subgraph AWSCloud["AWS Cloud"]
        subgraph VPC["VPC (10.0.0.0/16)"]
            subgraph Subnet1["Public Subnet 1 (10.0.1.0/24)"]
                ALB[Application Load Balancer] -.-> |Port 80| ASG1[EC2 t2.micro #1]
                ASG1 -->|Pull Image| ECR[ECR]
                ASG1 ==>|Setup| DOCKER1[Docker Instance 1]
            end
            
            subgraph Subnet2["Public Subnet 2 (10.0.2.0/24)"]
                ALB -.->|Port 80| ASG2[EC2 t2.micro #2]
                ASG2 -->|Pull Image| ECR
                ASG2 ==>|Setup| DOCKER2[Docker Instance 2]
            end
            
            IG[Internet Gateway] ==>RT[Route Table]
            RT ==>|0.0.0.0/0| PS1[Subnet 1]
            RT ==>|0.0.0.0/0| PS2[Subnet 2]
            
            ASG1 & ASG2 -.->|Port 6379| REDIS[Redis cache.t3.micro]
            ASG1 & ASG2 -.->|Port 5432| RDS[PostgreSQL db.t3.micro]
        end
        
        INET[Internet] ==>|Port 80| ALB
        CF ==>|Creates| VPC
    end
    
    subgraph SecurityGroups["Security Groups"]
        SG1[ALB SG - Port 80] -..-> ALB
        SG2[EC2 SG - Ports 80,22] -..-> ASG1
        SG2 -..-> ASG2
        SG3[Redis SG - Port 6379] -..-> REDIS
        SG4[RDS SG - Port 5432] -..-> RDS
    end
    
    subgraph IAMRoles["IAM & Roles"]
        EC2ROLE[EC2 Role] ==>|Assume| ASG1 & ASG2
        EC2ROLE ==>|Allow| ECRPULL[ECR Pull]
    end
    
    subgraph AutoScaling["Auto Scaling"]
        CPU[CPU 70%] -.->|Scale 2-4| ASG1 & ASG2
        HEALTH[Health Check /] -.->|30s| ALB
        ROLL[Rolling 1:1] -.->ASG1 & ASG2
    end

    subgraph DBGroups["Database Groups"]
        DBSUB[DB Subnet] ==>RDS
        REDISSUB[Redis Subnet] ==>REDIS
    end
